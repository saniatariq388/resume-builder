window.addEventListener('load', () => {
    ...
    if (profilePicture) {
      (document.getElementById('profilePic') as HTMLImageElement).src = profilePicture;
    }
  
  
  //(/\s+/g, '-') regits removes white spaces and if user name not provide it write user as name
    
    const username = name ? name.toLowerCase().replace(/\s+/g, '-') : 'user'; // Generate username from the name
    const baseUrl = 'http://127.0.0.1:5501/Dynamic_Resume/Preview_Resume/Dynamic_Resume.html'; // Your Vercel deployment URL
    const uniqueResumeUrl = `${baseUrl}?/${username}`; // for create unique URL
  ...
  document.getElementById('printBtn')!.addEventListener('click', () => {
    window.print(); // for trigger the print action
  });
